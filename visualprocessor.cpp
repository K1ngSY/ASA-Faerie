...
bool VisualProcessor::checkJoinCard(HWND Hwnd, int &Val_X, int &Val_Y)
{
    // 1. 截取整个窗口
    QImage fullImage;
    ...
    if (ri) {
        do {
            ...
            if (ws == QStringLiteral("JOIN") || ws == QStringLiteral("加入游戏") || ws == QStringLiteral("JOINGAME"))
            {
                // 取出这块文字的包围盒坐标
                int x1, y1, x2, y2;
                ...
                qDebug() << "checkJoinCard: 找到关键词" << ws << "坐标:" << Val_X << Val_Y;
                ocr.End();
                return true;
            }
...
cv::Mat VisualProcessor::QImageToCvMat(const QImage &inImage)
{
...
QImage VisualProcessor::CvMatToQImage(const cv::Mat &inMat)
{
...
bool VisualProcessor::ocrImage(const QImage &inputImage1,
...
bool VisualProcessor::analyzeGameWindow(HWND hwnd,
                                    QString &ocrResult1,
                                    QString &ocrResult2,
                                    QImage &pic1,
                                    QImage &pic2,
                                    QImage &fullImage)
{
    ...
    pic1 = img.copy(int(w*0.08),    int(h*0.01203),
                    int(w*0.2661),  int(h*0.02778));
    pic2 = img.copy(int(w*0.39531), int(h*0.17778),
                    int(w*0.209375),int(h*0.584259));

    // 转 cv::Mat 并拆通道
    cv::Mat m1 = QImageToCvMat(pic1);
    cv::Mat m2 = QImageToCvMat(pic2);
...
bool VisualProcessor::checkStartButton(HWND Hwnd) {
